...
function firstPage() {
	page = 1;
	window.location.href="/SXZZ/Back/openVedio?page="+page;
}
...
if(mess.length > 3) {
	...
	function checkMe(){
		...
		var str = (file.value).split(".mp4");
		var str1 = (file.value).split(".avi");
		var str2 = (file.value).split(".wmv");
		if(file.value.length <= 0) {
			alert("请选择要上传的文件！");
			return false;
		}else if(str.length > 1 || str1.length > 1 || str2.length > 1){
			XXX();
			return true;
		}else {
			alert("您只能上传mp4,avi或wmv格式的视频！");
			return false;
		}
	...
	function doDelete(id,name) {
		...
		if(res) {
			window.location.href="/SXZZ/Back/delVedio?id="+id+"&name="+name;
		}
	}
...
<body>
	...
	<div class="formbody">
		...
		<div class="upload">
			<form onsubmit="return checkMe()" action="/SXZZ/Back/upload_video"
				method="post" enctype="multipart/form-data">
				<input type="file" name="file" id="file"> <input
					type="submit" class="btn" value="确认上传">
			</form>
		</div>
		<div class="formtitle">
			<h3 style="color: red;">您能上传的视频格式包括mp4,avi和wmv,视频必须小于500M</h3>
		</div>
			...
			<div class="massage">
				<div class="header" onmousedown="MDown(massage_box);">
					...
					<span
						...
						src="/SXZZ/back/images/guanbi.png" alt="×" height="25px" width="25px" /></span>
				</div>
				<div
